...
impl ScyllaDb {
    ...
    pub fn get_trade_batch_values(
        ...
        queue_trade: &QueueTrade,
        trade_id: Id
    ) -> (i64, String, String, bool, String, i64) {
        let trade = Trade::new(
            trade_id,
    ...
    pub async fn exchange_balances(&self, queue_trade: QueueTrade) -> Result<(), Box<dyn Error>> {
        ...
        let mut order_1 = self.get_order(queue_trade.order_id_1).await.unwrap();
        let mut order_2 = self.get_order(queue_trade.order_id_2).await.unwrap();
        let trade_id = self.new_trade_id().await.unwrap();
